...
import { actions, messages } from "../utils/container";
...
export default class RichTextEditor extends Component {
      ...
      switch (message.type) {
        ...
        case messages.OFFSET_Y:
          let offsetY = Number.parseInt(
            Number.parseInt(data) + that.layout.y || 0
          );
  ...
  insertInitialContent(html) {
    html && this.setContentHTML(html);
  }
  ...
  preCode(style) {
    this.sendAction(actions.code, "result", style);
  }

  blockQuote(style) {
    this.sendAction(actions.blockquote, "result", style);
  }
