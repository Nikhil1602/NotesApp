function getContentCSS() {
  ...
  return `
    <style>
        ...
        blockquote{border-left: 6px solid #ddd;padding: 5px 10px 5px 10px;margin: 10px;}
        ...
        code {display: block;font-size: inherit;white-space: pre-wrap;color: inherit;}
...
<script>
    ...
    var _ = (function (exports) {
        ...
        function createCheckbox(end){
            ...
            var html = '<span contenteditable="true" class="x-todo-box"><input type="checkbox"></span>';
        ...
        var Actions = {
            ...
            tab: { result: function() { return exec('insertHTML', '&nbsp;&nbsp;'); }},
            orderedList: {
            ...
            quote: { 
                result: function(style) { 
                    var text = window.getSelection();

                    if(text != ''){
                        // var flag = exec(formatBlock, '<blockquote>');
                        // var node = anchorNode.nodeName === "BLOCKQUOTE" ? anchorNode : anchorNode.parentNode;
    
                        // if(node.nodeName === "BLOCKQUOTE"){
                            // style != undefined && node.setAttribute("style", style);
                            // var br = createElement("br");
            ...
            code: {
                ...
                if (node.nodeName === 'PRE'){
                    ...
                    // var html = "<code style='" + (style || '') + "' type='"+(type || '') +"'>" + node.innerHTML + "</code>";
                    // exec('insertHTML', html);
                    style != undefined && node.setAttribute("style", style);
            ...
            hiliteColor: {  state: function() { return queryCommandState('hiliteColor'); }, result: function(color) { 
                var text = window.getSelection();
                if(text != ''){
            ...
            video: {
                result: function(url, style) {
                    if (url) {
                        ...
                        var html = "<br><div style='display:flex; justify-content:center'><iframe style='"+ (style || '')+"' src='"+ url +"'></iframe></div><br><br>";
                        exec('insertHTML', html);
                        Actions.UPDATE_HEIGHT();
                    }
                }
